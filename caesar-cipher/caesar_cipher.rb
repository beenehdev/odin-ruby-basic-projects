...
def cipher_numcode_shifter(precoded_letter, shift_by_int)
  ...
  postcoded_letter -= 26 if postcoded_letter > 26
  postcoded_letter += 26 if postcoded_letter < 1
...
def caesar_cipher(string_input, shift_value_input)
  ...
  string_input.each_char do |ch|
    was_upcase = false
    was_upcase = true if high_abc_array.include?(ch)
    ...
    result_letter = if was_upcase
                      low_abc_array[postcoded_letter - 1].upcase
                    else
                      low_abc_array[postcoded_letter - 1]
                    end
...
string_input, shift_value_input = user_input
